...
struct cmd {
  int type;          //  ' ' (exec), | (pipe), '<' or '>' for redirection, ; and &
};
...
struct execcmd {
  ...
  char *argv[MAXARGS];
  //char *argv[MAXARGS];
 // char *eargv[MAXARGS];  // arguments to the command to be exec-ed
};
...
runcmd(struct cmd *cmd)
{
  ...
  struct Sqlcmd *scmd;//for ; 
  struct Paracmd *pacmd;// for &
  ...
  case ' ':
    ...
    // Your code here ...
    execvp(ecmd->argv[0],ecmd->argv); //added this line to execute the commands
